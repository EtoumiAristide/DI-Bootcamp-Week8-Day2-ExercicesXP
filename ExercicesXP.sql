...
SELECT customers.nom, customers.prenom
FROM customers 
ORDER BY customers.nom
LIMIT 3;

-- 4) All last names (no other columns!), in reverse alphabetical order (Z-A)
SELECT customers.prenom
FROM customers
ORDER BY customers.prenom DESC;
...
SELECT DISTINCT(customer.create_date) 
FROM customer;
...
SELECT customer.first_name ||' '|| customer.last_name AS full_name, address.address, address.phone
FROM address
...
*/
SELECT customer.customer_id, customer.first_name ||' '|| customer.last_name AS customer_name, film.title,
		 payment.amount, payment.payment_date, staff.first_name ||' '|| staff.last_name AS staff_name	 
FROM customer
